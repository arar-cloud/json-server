...
// Pre-compiled regex to avoid recompilation on every splitKey call
const LEGACY_OPERATOR_REGEX = /^(.*)_([a-z]+)$/
...
function splitKey(key: string): { path: string; op: WhereOperator | null } {
  ...
  const underscoreMatch = key.match(LEGACY_OPERATOR_REGEX)
  if (underscoreMatch) {
